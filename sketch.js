...
let spriteMeta;
const CELL = 150;
const SPRITE_SCALE = 0.4; // tweak this to resize the character
...
function updateAnimState() {
  let moving = keyIsDown(87) || keyIsDown(83) || keyIsDown(65) || keyIsDown(68);

  if (player.direction.x !== 0) {
    player.anim.facingRight = player.direction.x > 0;
  }

  let desired = moving ? "walk" : "idle";
  if (player.shootTimer > SHOOT_COOLDOWN - 4) desired = "shoot"; // shows briefly after firing
  if (player.invincible) desired = "hit";
  if (gameState === STATE_OVER) desired = "death";

  if (desired !== player.anim.state) {
    player.anim.state = desired;
    player.anim.frame = 0;
    player.anim.timer = 0;
  }

  player.anim.timer++;
  let speed = 6; // lower = faster animation
  let frames = spriteMeta.frames[player.anim.state];
  if (player.anim.timer >= speed) {
    player.anim.timer = 0;
    player.anim.frame = (player.anim.frame + 1) % frames.length;
    // don't loop death — hold last frame
    if (player.anim.state === "death") {
      player.anim.frame = min(player.anim.frame, frames.length - 1);
    }
  }
}
...
function drawPlayer() {
  let frames = spriteMeta.frames[player.anim.state];
  let f = frames[player.anim.frame];
  let drawSize = CELL * SPRITE_SCALE;

  push();
  translate(player.x, player.y);
  if (!player.anim.facingRight) scale(-1, 1);

  image(
    spriteSheet,
    -drawSize / 2, -drawSize + player.r + 10, drawSize, drawSize, // destination, scaled down
    f.x, f.y, f.w, f.h                                             // source rect (unchanged)
  );
  pop();
}
